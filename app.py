...
def helper(dis):
    ...
    med = med.values[0] if not med.empty else "No medication available"
    # Convert the string representation of the list into an actual list
    med = eval(med) if isinstance(med, str) else []

    die = diets[diets['Disease'] == dis]['Diet']
    die = die.values[0] if not die.empty else "No medication available"
    # Convert the string representation of the list into an actual list
    die = eval(die) if isinstance(die, str) else []
...
def index():
    ...
    if request.method == "POST":
        selected_symptoms = request.form.getlist("symptoms")  # Get selected symptoms
    return render_template("index.html", symptoms_dict=symptoms_dict, selected_symptoms=selected_symptoms)
...
def home():
    if request.method == 'POST':
        ...
        if not symptoms or symptoms.strip().lower() == "symptoms":
            message = "Please enter valid symptoms. Do not leave the input empty."
        ...
        if invalid_symptoms:
            message = f"Invalid symptom(s): {', '.join(invalid_symptoms)}. Please check your spelling."
            return render_template('index.html', message=message, symptoms_dict=symptoms_dict)
